...
class Baseline:
    ...
    def __init__(self, mean=None, sigma=None, samples=0, made=None, learned=None, roasts=None):
        self.mean = mean or {}
        self.sigma = sigma or {}
    ...
    def save(self, path):
        with open(path, "w") as fh:
            json.dump({"version": CALIB_VERSION, "made": self.made, "samples": self.samples,
                       "mean": self.mean, "sigma": self.sigma, "learned": self.learned, "roasts": self.roasts}, fh, indent=1, sort_keys=True)
    ...
    def load(path):
        ...
        if data.get("version") != CALIB_VERSION:
            return Baseline(learned=data.get("learned", {}), roasts=data.get("roasts", {}))
        return Baseline(data.get("mean", {}), data.get("sigma", {}),
                        data.get("samples", 0), data.get("made"),
                        data.get("learned", {}), data.get("roasts", {}))
...
def feature_vector(face, hands, body, tongue, gesture, m):
    """Compact numeric representation used by the optional learnable reaction layer."""
    return np.array([
        float(m.get("jaw", 0.0)), float(m.get("z_jaw", 0.0)),
        float(m.get("z_sneer", 0.0)), float(m.get("z_brow", 0.0)),
        float(m.get("z_frown", 0.0)), float(m.get("z_lip", 0.0)),
        float(m.get("z_squint", 0.0)), float(m.get("turn", 0.0)),
        float(tongue), float(gesture), float(len(hands)),
        float(bool(body and body.elbows_up)),
    ], dtype=np.float32)


def predict_learned(learned, vec):
    """Nearest learned reaction. Returns (label, distance) or (None, inf)."""
    if not learned:
    ...
    for label, info in learned.items():
        ...
        if center.shape != vec.shape:
            continue
        # Scale channels so a large sigma score does not dominate everything.
        scale = np.array([0.15, 3.0, 3.0, 3.0, 3.0, 3.0, 3.0, 0.25,
                          0.5, 0.08, 1.0, 1.0], dtype=np.float32)
        d = np.sqrt(np.mean(((vec - center) / scale) ** 2))
        if d < best_dist:
            best_label, best_dist = label, float(d)
    # Only accept a learned reaction when it is reasonably close to its examples.
    return (best_label, best_dist) if best_dist <= 1.55 else (None, best_dist)
...
def main():
    ...
    print("Running. q quit, d HUD, l learn, e edit roasts, x delete learned reaction, c recalibrate, 1-9 0 - = [ ] test")
        ...
        while True:
            ...
            if learn_state == "capture" and face is not None:
                ...
                if len(learn_samples) >= LEARN_TOTAL:
                    center = np.mean(np.stack(learn_samples), axis=0)
                    learned[learn_name] = {
            ...
            if roast_state:
                ...
                if key == 27:
                    delete_labels, delete_index = [], 0
                elif key in (2490368, 82):  # Up arrow (platform-dependent OpenCV code)
                    delete_index = (delete_index - 1) % len(delete_labels)
                elif key in (2621440, 84):  # Down arrow (platform-dependent OpenCV code)
                    delete_index = (delete_index + 1) % len(delete_labels)
                elif key in (10, 13):
